...
class WeeklyNewsletterBatch:
    def __init__(self, chunk_size: int = 100, max_retry_count: int = 3, ses_client: SESClient | None = None):
        """
        클래스 초기화
        ...
        if self.ses_client is None:
            raise ValueError("SES client is required")
    ...
    def _get_target_user_chunks(self) -> list[list[dict]]:
        ...
        try:
            target_users = list(User.objects.filter(
                is_active=True,
                email__isnull=False,
            ).values('id', 'email', 'username').distinct('email'))
    ...
    def _get_users_weekly_stats_chunk(self, user_ids: list[int]) -> dict[int, dict]:
        ...
        try:
            ...
            logger.warning(f"Failed to get users weekly stats: {e}")
            return {} # 예외 발생 시 빈 딕셔너리 반환


    def _get_users_weekly_trend_chunk(self, user_ids: list[int]) -> dict[int, UserWeeklyTrend]:
    ...
    def _build_newsletters(self, user_chunk: list[dict], weekly_trend_html: str) -> list[Newsletter]:
        ...
        try:
            ...
            # 개인화를 위한 데이터 일괄 조회
            users_weekly_trends_chunk = self._get_users_weekly_trend_chunk(user_ids)
            users_weekly_stats_chunk = self._get_users_weekly_stats_chunk(user_ids)
            expired_token_user_ids = self._get_expired_token_user_ids(user_ids)

            for user in user_chunk:
                try:
                    # user_id 키의 딕셔너리에서 개인 데이터 조회
                    user_weekly_trend = users_weekly_trends_chunk.get(user['id'])
                    user_weekly_stats = users_weekly_stats_chunk.get(user['id'], {'posts': 0, 'views': 0, 'likes': 0})
                    ...
                    continue # 개인 build 실패해도 청크는 계속 진행
    ...
    def _update_weekly_trend_result(self) -> None:
        ...
        try:
            ...
            logger.info(f"Updated WeeklyTrend #{self.weekly_info['newsletter_id']} as processed")
    ...
    def _update_user_weekly_trend_results(self, success_user_ids: list[int]) -> None:
        ...
        try:
            ...
            for chunk_index, user_chunk in enumerate(target_user_chunks, 1):
                ...
                try:
                    ...
                    newsletters = self._build_newsletters(user_chunk, weekly_trend_html)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(f"Failed to initialize SES client for sending newsletter: {e}")
        raise e from e
    
    # 배치 실행
    WeeklyNewsletterBatch(ses_client=ses_client)._run()