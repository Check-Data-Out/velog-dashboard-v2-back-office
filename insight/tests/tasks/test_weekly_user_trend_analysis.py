...
class TestUserWeeklyAnalyzer:
    ...
    async def test_fetch_data_handles_token_expired_error(
        self, mock_stats, mock_posts, mock_users, analyzer, mock_context
    ):
        ...
        with patch.object(analyzer, "logger") as mock_logger:
            ...
            assert result == []
            assert 1 in analyzer.expired_token_users
            mock_logger.warning.assert_any_call("User %s token expired - no today stats", 1)
